...
(function () {
    ...
    const setStatus = (connected, text) => {
        ...
        if (connectStatusText) {
            connectStatusText.textContent = text || (connected ? 'connected' : 'offline');
        }
    };
    ...
    if (robotSocket) {
        ...
        robotSocket.on('robot_msg', (msg) => {
            const label = msg && (msg.msgName || msg.decoded || msg.rawPayload)
                ? (msg.msgName || msg.decoded || msg.rawPayload)
                : 'robot message';
            setStatus(true, label);

            if (msg && msg.telemetry && Number.isFinite(msg.telemetry.voltage)) {
                batteryInput.value = `${msg.telemetry.voltage.toFixed(2)} V`;
            }
            if (msg && msg.telemetry && Number.isFinite(msg.telemetry.energy)) {
